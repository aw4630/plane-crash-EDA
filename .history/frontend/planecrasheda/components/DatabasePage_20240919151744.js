...
function DatabasePage() {
  ...
  const [searchSubmitted, setSearchSubmitted] = useState(false); // Added for form submission tracking
  ...
  useEffect(() => {
    ...
    if (searchSubmitted || sortOption) {
      fetchIncidents();  // Fetch when form is submitted or sortOption changes
    }
  ...
  const handleSearchSubmit = (event) => {
    ...
    setIncidents([]);  // Clear current incidents when a new search is performed
    setSearchSubmitted(true);  // Trigger the form submission flag
  };

  const handleSortChange = (e) => {
    setSortOption(e.target.value);
    setIncidents([]);  // Clear current incidents on sort change
  };
  ...
  const loadMoreIncidents = async () => {
    try {
      ...
      setIncidents((prevIncidents) => [...prevIncidents, ...response.data]);  // Append more data
      setHasMore(response.data.length === limit);
    } catch (err) {
