...
function DatabasePage() {
  ...
  const [limit] = useState(50);  // Load 50 incidents at a time
  const [offset, setOffset] = useState(0);  // Track how many entries are loaded
  const [loading, setLoading] = useState(false);
  const [hasMore, setHasMore] = useState(true);

  // Fetch incidents based on the offset
  const fetchIncidents = async (newOffset = 0) => {
    try {
      setLoading(true);
      ...
      // Append new incidents to the existing list
      if (newOffset === 0) {
        setIncidents(response.data);  // First load or after a new search
      } else {
        setIncidents((prevIncidents) => [...prevIncidents, ...response.data]);  // Load more incidents
      }
  ...
  const handleSearchSubmit = (event) => {
    ...
    fetchIncidents(0);  // Fetch new search results from the first entry
  };

  const handleSortChange = (e) => {
    setSortOption(e.target.value);
    setOffset(0);  // Reset offset on sort change
    fetchIncidents(0);  // Fetch sorted results from the first entry
  };

  // Load more incidents when the "Load More" button is clicked
  const loadMoreIncidents = () => {
    const newOffset = offset + limit;
    setOffset(newOffset);  // Update the offset for the next batch
    fetchIncidents(newOffset);  // Fetch the next batch of incidents
  };
  ...
  <style>
    ...
    `}
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '5px' }}>
       ...
       {/* "Load More" Button */}
       {hasMore && !loading && (
        <button onClick={loadMoreIncidents} style={{ marginTop: '20px', padding: '10px', fontSize: '16px' }}>
          Load More
        </button>
      )}
      {loading && <p>Loading...</p>}
