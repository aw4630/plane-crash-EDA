...
export default async function handler(req, res) {
  const {
    ...
    destination,
    primaryCause,
    limit = 100,  // Default limit to 100 results per page
    offset = 0    // Default offset (start from the first row)
  } = req.query;
  ...
  try {
    ...
    let query = 'SELECT DISTINCT "Date", "Type", "Owner/operator", "Fatalities", "Location", "Phase", "Nature" FROM database';
    ...
    query += ` ${sortQuery} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;  // Add LIMIT and OFFSET
